...
struct ActionRequest {
  ...
  3:required string action_type (vt.in = "1", vt.in = "2" go.tag = 'json:"action_type" query:"action_type"')             // 操作类型（1-点赞，2-取消点赞）
}
...
struct FavoriteRequest {
  1:required string user_id (go.tag = 'json:"user_id" query:"user_id"')               // 用户id
